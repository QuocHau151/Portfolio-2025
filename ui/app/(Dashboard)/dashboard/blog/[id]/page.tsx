...
export default function EditBlog({
  ...
  const getBlogsByAuthor = useGetBlogsByAythorQuery();
  const blogId = getBlogsByAuthor.data?.payload.data.map((item) => item.id);

  useEffect(() => {
    // Chỉ kiểm tra khi data đã được tải
    if (getBlogsByAuthor.data && blogId) {
      if (!blogId.includes(Number(id))) {
        toast.error("Bạn không có quyền truy cập vào bài viết này.");
        router.push("/dashboard/blog");
      }
    }
  }, [blogId, id, getBlogsByAuthor.data]);
