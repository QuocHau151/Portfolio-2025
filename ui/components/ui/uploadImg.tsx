...
type InputProps = {
  className?: string;
  value?: IMGState[];
  onChange?: (files: IMGState[]) => void | Promise<void>;
  onFilesAdded?: (addedFiles: IMGState[]) => void | Promise<void>;
  disabled?: boolean;
  ...
  (
    ...
    const {
      ...
      onDrop: (acceptedFiles) => {
        ...
        if (files) {
          const addedFiles = files.map<IMGState>((file) => ({
            file,
          ...
          {value?.map(({ file, progress }, index) => (
            <div key={index} className={variants.image + " aspect-square"}>
              <Image
                ...
                src={imageUrls[index] || "/assets/images/loading.jpg"}
